...
absl::Status MontgomeryInt<T>::BatchFusedMulAddInPlace(
  ...
  size_t i = 0;
  auto* out = in1->data();
  const auto* lhs = in2.data();
  const auto* rhs = in3.data();
  const size_t size = in1->size();
  const Int r_mod_modulus = params->r_mod_modulus;
  const Int inv_modulus = params->inv_modulus;
  const Int modulus = params->modulus;
  const BigInt modulus_bigint = params->modulus_bigint;
  for (; i + 4 <= size; i += 4) {
    BigInt t0 = static_cast<BigInt>(r_mod_modulus) * out[i + 0].n_ +
                static_cast<BigInt>(lhs[i + 0].n_) * rhs[i + 0].n_;
    BigInt t1 = static_cast<BigInt>(r_mod_modulus) * out[i + 1].n_ +
                static_cast<BigInt>(lhs[i + 1].n_) * rhs[i + 1].n_;
    BigInt t2 = static_cast<BigInt>(r_mod_modulus) * out[i + 2].n_ +
                static_cast<BigInt>(lhs[i + 2].n_) * rhs[i + 2].n_;
    BigInt t3 = static_cast<BigInt>(r_mod_modulus) * out[i + 3].n_ +
                static_cast<BigInt>(lhs[i + 3].n_) * rhs[i + 3].n_;

    const Int u0 = static_cast<Int>(t0) * inv_modulus;
    const Int u1 = static_cast<Int>(t1) * inv_modulus;
    const Int u2 = static_cast<Int>(t2) * inv_modulus;
    const Int u3 = static_cast<Int>(t3) * inv_modulus;

    t0 += modulus_bigint * u0;
    t1 += modulus_bigint * u1;
    t2 += modulus_bigint * u2;
    t3 += modulus_bigint * u3;

    const Int t0_msb = static_cast<Int>(t0 >> Params::bitsize_int);
    const Int t1_msb = static_cast<Int>(t1 >> Params::bitsize_int);
    const Int t2_msb = static_cast<Int>(t2 >> Params::bitsize_int);
    const Int t3_msb = static_cast<Int>(t3 >> Params::bitsize_int);

    out[i + 0].n_ = (t0_msb >= modulus) ? t0_msb - modulus : t0_msb;
    out[i + 1].n_ = (t1_msb >= modulus) ? t1_msb - modulus : t1_msb;
    out[i + 2].n_ = (t2_msb >= modulus) ? t2_msb - modulus : t2_msb;
    out[i + 3].n_ = (t3_msb >= modulus) ? t3_msb - modulus : t3_msb;
  }
  for (; i < size; ++i) {
    BigInt t = static_cast<BigInt>(r_mod_modulus) * out[i].n_ +
               static_cast<BigInt>(lhs[i].n_) * rhs[i].n_;
    const Int u = static_cast<Int>(t) * inv_modulus;
    t += modulus_bigint * u;
    const Int t_msb = static_cast<Int>(t >> Params::bitsize_int);
    out[i].n_ = (t_msb >= modulus) ? t_msb - modulus : t_msb;
  }
  return absl::OkStatus();
}
...
absl::Status MontgomeryInt<T>::BatchFusedMulConstantAddInPlace(
  ...
  auto* out = in1->data();
  const auto* lhs = in2.data();
  const auto* constant_ptr = constant.data();
  const auto* constant_barrett_ptr = constant_barrett.data();
  const size_t size = in1->size();
  const Int modulus = params->modulus;
  const BigInt barrett_numerator = params->barrett_numerator;
  for (; i + 4 <= size; i += 4) {
    Int out0 = static_cast<Int>(
        (static_cast<BigInt>(constant_barrett_ptr[i + 0]) * lhs[i + 0].n_ +
         barrett_numerator * out[i + 0].n_) >>
        Params::bitsize_int);
    Int out1 = static_cast<Int>(
        (static_cast<BigInt>(constant_barrett_ptr[i + 1]) * lhs[i + 1].n_ +
         barrett_numerator * out[i + 1].n_) >>
        Params::bitsize_int);
    Int out2 = static_cast<Int>(
        (static_cast<BigInt>(constant_barrett_ptr[i + 2]) * lhs[i + 2].n_ +
         barrett_numerator * out[i + 2].n_) >>
        Params::bitsize_int);
    Int out3 = static_cast<Int>(
        (static_cast<BigInt>(constant_barrett_ptr[i + 3]) * lhs[i + 3].n_ +
         barrett_numerator * out[i + 3].n_) >>
        Params::bitsize_int);

    out[i + 0].n_ += lhs[i + 0].n_ * constant_ptr[i + 0] - out0 * modulus;
    out[i + 1].n_ += lhs[i + 1].n_ * constant_ptr[i + 1] - out1 * modulus;
    out[i + 2].n_ += lhs[i + 2].n_ * constant_ptr[i + 2] - out2 * modulus;
    out[i + 3].n_ += lhs[i + 3].n_ * constant_ptr[i + 3] - out3 * modulus;

    out[i + 0].n_ -= (out[i + 0].n_ >= modulus) ? modulus : 0;
    out[i + 1].n_ -= (out[i + 1].n_ >= modulus) ? modulus : 0;
    out[i + 2].n_ -= (out[i + 2].n_ >= modulus) ? modulus : 0;
    out[i + 3].n_ -= (out[i + 3].n_ >= modulus) ? modulus : 0;
  }
  for (; i < size; ++i) {
    Int reduced = static_cast<Int>(
        (static_cast<BigInt>(constant_barrett_ptr[i]) * lhs[i].n_ +
         barrett_numerator * out[i].n_) >>
        Params::bitsize_int);
    out[i].n_ += lhs[i].n_ * constant_ptr[i] - reduced * modulus;
    out[i].n_ -= (out[i].n_ >= modulus) ? modulus : 0;
  }
  return absl::OkStatus();
}

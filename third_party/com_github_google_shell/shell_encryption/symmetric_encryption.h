...
class SymmetricRlweCiphertext {
 ...
 public:
  ...
  rlwe::StatusOr<SymmetricRlweCiphertext> operator*(
      const ConstantPolynomial<ModularInt>& that) const {
    SymmetricRlweCiphertext out = *this;
    RLWE_RETURN_IF_ERROR(out.AbsorbInPlace(that));
    return out;
  }

  absl::Status AbsorbInPlace(const ConstantPolynomial<ModularInt>& that) {
    for (auto& component : this->c_) {
      RLWE_RETURN_IF_ERROR(component.MulConstantInPlace(that, modulus_params_));
    }
    error_ *= error_params_->B_plaintext();
    return absl::OkStatus();
  }
  ...
  const std::vector<Polynomial<ModularInt>>& Components() const { return c_; }

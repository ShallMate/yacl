...
std::atomic<std::uint64_t>& InverseNttTimingUs() {
  static auto* value = new std::atomic<std::uint64_t>(0);
  return *value;
}

std::atomic<std::uint64_t>& BitDecomposeMatrixTimingUs() {
  static auto* value = new std::atomic<std::uint64_t>(0);
  return *value;
}

std::atomic<std::uint64_t>& ExportCoeffsTimingUs() {
  static auto* value = new std::atomic<std::uint64_t>(0);
  return *value;
}

std::atomic<std::uint64_t>& DigitExtractTimingUs() {
  static auto* value = new std::atomic<std::uint64_t>(0);
  return *value;
}

std::atomic<std::uint64_t>& ConvertNttTimingUs() {
  static auto* value = new std::atomic<std::uint64_t>(0);
  return *value;
}

std::atomic<std::uint64_t>& FusedMulAddTimingUs() {
  static auto* value = new std::atomic<std::uint64_t>(0);
  return *value;
}

template <typename ModularInt>
struct BitDecomposeLookupCacheKey {
  const typename ModularInt::Params* modulus_params = nullptr;
  Uint64 log_decomposition_modulus = 0;

  bool operator<(const BitDecomposeLookupCacheKey& other) const {
    return std::tie(modulus_params, log_decomposition_modulus) <
           std::tie(other.modulus_params, other.log_decomposition_modulus);
  }
};

template <typename ModularInt>
std::mutex& BitDecomposeLookupCacheMutex() {
  static auto* mu = new std::mutex();
  return *mu;
}

template <typename ModularInt>
std::map<BitDecomposeLookupCacheKey<ModularInt>,
         std::shared_ptr<const std::vector<ModularInt>>>&
BitDecomposeLookupCache() {
  static auto* cache = new std::map<BitDecomposeLookupCacheKey<ModularInt>,
                                    std::shared_ptr<const std::vector<ModularInt>>>();
  return *cache;
}

template <typename ModularInt>
rlwe::StatusOr<std::shared_ptr<const std::vector<ModularInt>>>
GetBitDecomposeLookup(const typename ModularInt::Params* modulus_params,
                      Uint64 log_decomposition_modulus) {
  BitDecomposeLookupCacheKey<ModularInt> key{
      .modulus_params = modulus_params,
      .log_decomposition_modulus = log_decomposition_modulus,
  };
  {
    std::lock_guard<std::mutex> lock(BitDecomposeLookupCacheMutex<ModularInt>());
    const auto it = BitDecomposeLookupCache<ModularInt>().find(key);
    if (it != BitDecomposeLookupCache<ModularInt>().end()) {
      return it->second;
    }
  }

  const auto lookup_size = std::size_t{1} << log_decomposition_modulus;
  auto mutable_lookup = std::make_shared<std::vector<ModularInt>>();
  mutable_lookup->reserve(lookup_size);
  for (std::size_t value = 0; value < lookup_size; ++value) {
    RLWE_ASSIGN_OR_RETURN(
        auto modular_value,
        ModularInt::ImportInt(static_cast<typename ModularInt::Int>(value),
                              modulus_params));
    mutable_lookup->push_back(std::move(modular_value));
  }
  std::shared_ptr<const std::vector<ModularInt>> lookup = mutable_lookup;

  {
    std::lock_guard<std::mutex> lock(BitDecomposeLookupCacheMutex<ModularInt>());
    auto [it, inserted] =
        BitDecomposeLookupCache<ModularInt>().emplace(key, lookup);
    if (!inserted) {
      lookup = it->second;
    }
  }
...
BitDecomposeAndMatrixMultiply(
  ...
  std::vector<typename ModularInt::Int> ciphertext_coeffs;
  ciphertext_coeffs.reserve(coefficients.size());
  for (const auto& coefficient : coefficients) {
    ciphertext_coeffs.push_back(coefficient.ExportInt(modulus_params));
  }
  ExportCoeffsTimingUs().fetch_add(
      static_cast<std::uint64_t>(
          std::chrono::duration_cast<std::chrono::microseconds>(
              std::chrono::steady_clock::now() - export_started_at)
              .count()),
  ...
  const auto mask =
      ...
      1;
  const std::vector<ModularInt>* digit_lookup = nullptr;
  if (log_decomposition_modulus <= 16) {
    RLWE_ASSIGN_OR_RETURN(
        auto lookup,
        GetBitDecomposeLookup<ModularInt>(modulus_params,
                                          log_decomposition_modulus));
    digit_lookup = lookup.get();
  }

  std::vector<Polynomial<ModularInt>> result(
      2, Polynomial<ModularInt>(coefficients.size(), modulus_params));
  const auto& row0 = matrix[0];
  const auto& row1 = matrix[1];
  const std::size_t num_coeffs = ciphertext_coeffs.size();
  for (size_t i = 0; i < row0.size(); i++) {
    std::vector<ModularInt> digit_coeffs;
    digit_coeffs.reserve(num_coeffs);
    const auto digit_extract_started_at = std::chrono::steady_clock::now();
    if (digit_lookup != nullptr) {
      for (size_t j = 0; j < num_coeffs; ++j) {
        const auto coefficient_part = ciphertext_coeffs[j] & mask;
        digit_coeffs.push_back(
            (*digit_lookup)[static_cast<std::size_t>(coefficient_part)]);
      ...
      for (size_t j = 0; j < num_coeffs; ++j) {
        ...
        RLWE_ASSIGN_OR_RETURN(
            auto modular_value,
            ModularInt::ImportInt(coefficient_part, modulus_params));
        digit_coeffs.push_back(std::move(modular_value));
        ciphertext_coeffs[j] >>= log_decomposition_modulus;
      }
    }
    DigitExtractTimingUs().fetch_add(
        static_cast<std::uint64_t>(
            std::chrono::duration_cast<std::chrono::microseconds>(
                std::chrono::steady_clock::now() - digit_extract_started_at)
                .count()),
        std::memory_order_relaxed);
    ...
    Polynomial<ModularInt> ntt_part = Polynomial<ModularInt>::ConvertToNtt(
        std::move(digit_coeffs), ntt_params, modulus_params);
    ConvertNttTimingUs().fetch_add(
        static_cast<std::uint64_t>(
            std::chrono::duration_cast<std::chrono::microseconds>(
                std::chrono::steady_clock::now() - convert_started_at)
                .count()),
        std::memory_order_relaxed);
    const auto fused_started_at = std::chrono::steady_clock::now();
    RLWE_RETURN_IF_ERROR(
        result[0].FusedMulAddInPlace(ntt_part, row0[i], modulus_params));
    RLWE_RETURN_IF_ERROR(
        result[1].FusedMulAddInPlace(ntt_part, row1[i], modulus_params));
    FusedMulAddTimingUs().fetch_add(
        static_cast<std::uint64_t>(
            std::chrono::duration_cast<std::chrono::microseconds>(
                std::chrono::steady_clock::now() - fused_started_at)
                .count()),
        std::memory_order_relaxed);
  }
...
void ResetRelinearizationApplyTiming() {
  InverseNttTimingUs().store(0, std::memory_order_relaxed);
  BitDecomposeMatrixTimingUs().store(0, std::memory_order_relaxed);
  ExportCoeffsTimingUs().store(0, std::memory_order_relaxed);
  DigitExtractTimingUs().store(0, std::memory_order_relaxed);
  ConvertNttTimingUs().store(0, std::memory_order_relaxed);
  FusedMulAddTimingUs().store(0, std::memory_order_relaxed);
}

RelinearizationApplyTiming GetRelinearizationApplyTiming() {
  return {
      .inverse_ntt_us = InverseNttTimingUs().load(std::memory_order_relaxed),
      .bit_decompose_matrix_us =
          BitDecomposeMatrixTimingUs().load(std::memory_order_relaxed),
      .export_coeffs_us = ExportCoeffsTimingUs().load(std::memory_order_relaxed),
      .digit_extract_us = DigitExtractTimingUs().load(std::memory_order_relaxed),
      .convert_ntt_us = ConvertNttTimingUs().load(std::memory_order_relaxed),
      .fused_muladd_us = FusedMulAddTimingUs().load(std::memory_order_relaxed),
  };
}
...
RelinearizationKey<ModularInt>::RelinearizationKeyPart::ApplyPartTo(
  ...
  const auto inverse_ntt_started_at = std::chrono::steady_clock::now();
  ...
  InverseNttTimingUs().fetch_add(
      static_cast<std::uint64_t>(
          std::chrono::duration_cast<std::chrono::microseconds>(
              std::chrono::steady_clock::now() - inverse_ntt_started_at)
              .count()),
      std::memory_order_relaxed);

  const auto decompose_started_at = std::chrono::steady_clock::now();
  auto result = BitDecomposeAndMatrixMultiply<ModularInt>(
      ciphertext_coefficients, matrix_, modulus_params, ntt_params,
      log_decomposition_modulus_);
  BitDecomposeMatrixTimingUs().fetch_add(
      static_cast<std::uint64_t>(
          std::chrono::duration_cast<std::chrono::microseconds>(
              std::chrono::steady_clock::now() - decompose_started_at)
              .count()),
      std::memory_order_relaxed);
...
RelinearizationKey<ModularInt>::ApplyTo(
  ...
  const auto& ciphertext_components = ciphertext.Components();
  RLWE_ASSIGN_OR_RETURN(
      auto result,
      relinearization_key_[0].ApplyPartTo(ciphertext_components[1],
                                          modulus_params_, ntt_params_));
  ...
  for (size_t i = 1; i < relinearization_key_.size(); i++) {
    ...
    RLWE_ASSIGN_OR_RETURN(auto result_part,
                          relinearization_key_[i].ApplyPartTo(
                              ciphertext_components[i + 1], modulus_params_,
                              ntt_params_));
  ...
  RLWE_RETURN_IF_ERROR(result[0].AddInPlace(ciphertext_components[0],
                                            modulus_params_));

...
class Polynomial {
 ...
 private:
  ...
  void IterativeCooleyTukey(
      ...
      const ModularIntParams* modular_params) {
    auto* coeffs = coeffs_.data();
    const int len = Len();
    const auto* psis = psis_bitrev_constant.data();
    int index_psi = 1;
    for (int i = static_cast<int>(log_len_) - 1; i >= 0; i--) {
      ...
      for (int k = 0; k < len; k += m) {
        ...
        for (int j = 0; j < half_m; ++j) {
          auto& lower = coeffs[k + j];
          auto& upper = coeffs[k + j + half_m];
          const auto lower_raw = lower.GetMontgomeryRepresentation();
          const auto t_raw = ModularInt::MulConstantRaw(
              upper.GetMontgomeryRepresentation(), psi_constant,
              psi_constant_barrett, modular_params);
          if (i & 1) {
            lower = ModularInt(ModularInt::LazyAddRaw(lower_raw, t_raw));
            upper =
                ModularInt(ModularInt::LazySubRaw(lower_raw, t_raw,
                                                  modular_params));
          } else {
            lower = ModularInt(
                ModularInt::AddRaw(lower_raw, t_raw, modular_params));
            upper = ModularInt(
                ModularInt::SubRaw(lower_raw, t_raw, modular_params));
          }
        }
        index_psi++;
      }
  ...
  void IterativeGentlemanSande(
    ...
    auto* coeffs = coeffs_.data();
    const int len = Len();
    const auto* psis_inv = psis_inv_bitrev_constant.data();
    int index_psi_inv = 0;
    for (int i = 0; i < static_cast<int>(log_len_); i++) {
      ...
      for (int k = 0; k < len; k += m) {
        const auto& [psi_inv_constant, psi_inv_constant_barrett] =
            psis_inv[index_psi_inv];
        for (int j = 0; j < half_m; j++) {
          auto& lower = coeffs[k + j];
          auto& upper = coeffs[k + j + half_m];
          // The Gentleman-Sande butterfly operation.
          const ModularInt t = upper;
          ModularInt u = lower;
          lower.AddInPlace(t, modular_params);

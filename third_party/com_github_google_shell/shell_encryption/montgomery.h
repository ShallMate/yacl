...
class ABSL_MUST_USE_RESULT MontgomeryInt {
 public:
  ...
  static Int MulConstantRaw(const Int& value, const Int& constant,
                            const Int& constant_barrett, const Params* params) {
    Int out = static_cast<Int>(
        (static_cast<BigInt>(constant_barrett) * value) >> Params::bitsize_int);
    Int result = value * constant - out * params->modulus;
    result -= (result >= params->modulus) ? params->modulus : 0;
    return result;
  }

  static Int AddRaw(const Int& lhs, const Int& rhs, const Params* params) {
    return params->BarrettReduce(lhs + rhs);
  }

  static Int SubRaw(const Int& lhs, const Int& rhs, const Params* params) {
    return params->BarrettReduce(lhs + (params->modulus - rhs));
  }

  static Int LazyAddRaw(const Int& lhs, const Int& rhs) { return lhs + rhs; }

  static Int LazySubRaw(const Int& lhs, const Int& rhs, const Params* params) {
    return lhs + (params->modulus - rhs);
  }
  ...
  MontgomeryInt& MulConstantInPlace(const Int& constant,
                                    ...
                                    const Params* params) {
    n_ = MulConstantRaw(n_, constant, constant_barrett, params);
    return *this;
  }
